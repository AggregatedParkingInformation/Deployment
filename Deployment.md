...
4. `cd` in both directories (`system` and `parking-service`) and run `docker compose up -d` in each of them
...
> NOTE: On unix systems it might be required to run `dos2unix` on `parking-service/initdb/backup.sql`!
